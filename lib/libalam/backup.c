...
int _alam_backup_split(const char *string, char **file, char **hash)
{
...
{
	char *file = NULL;
	_alam_backup_split(string, &file, NULL);
	return(file);
}
...
{
	char *hash = NULL;
	_alam_backup_split(string, NULL, &hash);
	return(hash);
}

/* Look for a filename in a ampkg_t.backup list.  If we find it,
...
char *_alam_needbackup(const char *file, const alam_list_t *backup)
{
	...
	for(lp = backup; lp; lp = lp->next) {
		...
		/* no hash found */
		if(!_alam_backup_split((char *)lp->data, &filename, &hash)) {
			FREE(filename);
			continue;
		}
