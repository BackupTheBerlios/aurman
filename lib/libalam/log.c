...
/** \addtogroup alam_log Logging Functions
 * @brief Functions to log using libalam
 * @{
 */
 ...
 */
int SYMEXPORT alam_logaction(char *fmt, ...)
{
	int ret;
	va_list args;
	...
	if(handle->logstream == NULL) {
		...
		if(handle->logstream == NULL) {
			if(errno == EACCES) {
				am_errno = AM_ERR_BADPERMS;
			} else if(errno == ENOENT) {
				am_errno = AM_ERR_NOT_A_DIR;
			} else {
				am_errno = AM_ERR_SYSTEM;
			}
		return(-1);
		}
	}
	...
	va_start(args, fmt);
	ret = _alam_logaction(handle->usesyslog, handle->logstream, fmt, args);
	va_end(args);
...
void _alam_log(amloglevel_t flag, char *fmt, ...)
{
	va_list args;
	alam_cb_log logcb = alam_option_get_logcb();
